...
# - 2: 再忽略“隔一节”的近邻（通常最常用）
# - 3: 更激进（会更乐观，但一般仍合理）
COLLISION_IGNORE_KINEMATIC_DISTANCE = 2


# 自动调参：从 MIN 到 MAX 逐步增大 K，直到 q_ref 在该过滤策略下无碰撞。
# 这对“URDF collision mesh 在关节处有轻微重叠”的模型非常有效，否则经常 reachable=0。
AUTO_TUNE_COLLISION_IGNORE_DISTANCE = True
COLLISION_IGNORE_KINEMATIC_DISTANCE_MIN = 1
COLLISION_IGNORE_KINEMATIC_DISTANCE_MAX = 8
...
def chain_joint_ids_from_frame(model: pin.Model, frame_name: str):
    """从 frame 往上回溯得到关节链（root->...->parentJoint）。

    兼容不同 Pinocchio 版本：
    - 新版本：Frame.parentJoint
    - 旧版本：Frame.parent（已 deprecated，会产生 warning）
    """
    ...
    fr = model.frames[fid]
    jid = getattr(fr, "parentJoint", None)
    if jid is None:
        jid = fr.parent  # 兼容旧版本
...
def _set_collision_pairs(collision_model, pairs):
    """安全写回 collisionPairs（兼容不同 Pinocchio 版本实现）。"""
    try:
        collision_model.collisionPairs = list(pairs)
    except Exception:
        collision_model.collisionPairs.clear()
        collision_model.collisionPairs.extend(list(pairs))


def autotune_collision_ignore_distance(model, data, collision_model, q_ref: np.ndarray,
                                      k_min: int, k_max: int,
                                      debug_print: bool = True):
    """自动选择一个 k，使得 q_ref 在该过滤策略下无碰撞。

    目的：解决“加碰撞后 reachable=0”最常见根因：
    - URDF 的 collision mesh 在关节连接处轻微重叠/贴近
    - addAllCollisionPairs() 会把相邻/近邻连杆也纳入检测
    - 导致几乎任意姿态都被判碰撞

    返回：
      (k_selected, removed, collision_data, ok_at_qref)
    """
    pairs0 = list(getattr(collision_model, "collisionPairs", []))
    if not pairs0:
        cd = collision_model.createData()
        ok = is_collision_free(model, data, collision_model, cd, q_ref, True)
        return int(k_min), 0, cd, ok

    k_min = int(max(1, k_min))
    k_max = int(max(k_min, k_max))

    for k in range(k_min, k_max + 1):
        _set_collision_pairs(collision_model, pairs0)
        removed = prune_collision_pairs_by_kinematic_distance(model, collision_model, k)
        cd = collision_model.createData()
        ok = is_collision_free(model, data, collision_model, cd, q_ref, True)
        if debug_print:
            print(f"[INFO] autotune K={k}: pairs={len(collision_model.collisionPairs)} removed={removed} | q_ref collision-free={ok}")
        if ok:
            return k, removed, cd, ok

    # 兜底：即使到 k_max 仍碰撞，也返回当前设置，方便进一步 debug
    _set_collision_pairs(collision_model, pairs0)
    removed = prune_collision_pairs_by_kinematic_distance(model, collision_model, k_max)
    cd = collision_model.createData()
    ok = is_collision_free(model, data, collision_model, cd, q_ref, True)
    return k_max, removed, cd, ok
...
def main():
    ...
    k_ignore_selected = int(COLLISION_IGNORE_KINEMATIC_DISTANCE)
    try:
        if hasattr(collision_model, "addAllCollisionPairs"):
            collision_model.addAllCollisionPairs()

        q_ref0 = pin.neutral(model)

        # 自动调 K：优先保证 q_ref 至少是 collision-free，否则后面 workspace 很可能直接变 0
        if AUTO_TUNE_COLLISION_IGNORE_DISTANCE:
            k_sel, removed, collision_data_main, ok_ref = autotune_collision_ignore_distance(
                model, data, collision_model, q_ref0,
                COLLISION_IGNORE_KINEMATIC_DISTANCE_MIN,
                COLLISION_IGNORE_KINEMATIC_DISTANCE_MAX,
                debug_print=True,
            )
        else:
            k_sel = int(COLLISION_IGNORE_KINEMATIC_DISTANCE)
            removed = prune_collision_pairs_by_kinematic_distance(model, collision_model, k_sel)
            collision_data_main = collision_model.createData()
            ok_ref = is_collision_free(model, data, collision_model, collision_data_main, q_ref0, True)

        print(f"[INFO] Collision geom count: {collision_model.ngeoms} | pairs: {len(collision_model.collisionPairs)} | removed(<=K): {removed} | K_selected: {k_sel}")
        print(f"[INFO] q_ref collision-free under current pair filtering? {ok_ref}")

        # 若 q_ref 仍碰撞，直接把冲突对打印出来：下一步要么继续提高 K，要么用 SRDF 屏蔽这些对，要么修 collision mesh
        if (not ok_ref) and DEBUG_PRINT_REF_COLLISIONS:
            print("[DEBUG] Collisions at q_ref (show first pairs):")
            debug_print_collisions(model, data, collision_model, collision_data_main, q_ref0, DEBUG_COLLISION_PRINT_LIMIT)

        # 把最终选中的 K 写回，供后续 worker 使用
        k_ignore_selected = k_sel
    ...
    _PAR_CFG = {
        ...
        "collision_ignore_kinematic_distance": int(k_ignore_selected),
    }
